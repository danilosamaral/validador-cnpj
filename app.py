...
def extrair_dados_completos(pdf_file):
    ...
    # 1. Extrair CNPJ (aceita formatos com ou sem pontuação)
    match_cnpj = re.search(r'(\d{2}\.?\d{3}\.?\d{3}\/?\d{4}-?\d{2})', texto_completo)
    if match_cnpj:
        cnpj_raw = match_cnpj.group(1)
        dados['cnpj'] = cnpj_raw  # mantém versão original extraída (formatada ou não)

    # 2. Extrair Natureza Jurídica (captura versões com ou sem traço)
    # procura por padrão numérico comum (ex: 123-4 ou 1234) próximo ao texto indicado
    match_nj = re.search(r"CÓDIGO E DESCRIÇÃO DA NATUREZA JURÍDICA.*?([0-9\-]{3,6})", texto_completo, re.DOTALL | re.IGNORECASE)
    if match_nj:
        dados['nat_jur'] = match_nj.group(1).strip()

    # 3. Extrair CNAE Principal (aceita com ou sem pontuação)
    # formato padrão CNAE: 12.34-5-67 -> equivalente a 7 dígitos; capturamos variantes
    match_cnae_p = re.search(r"CÓDIGO E DESCRIÇÃO DA ATIVIDADE ECONÓMICA PRINCIPAL.*?(\d{2}\.?\d{2}-?\d-?\d{2}|\d{7})", texto_completo, re.DOTALL | re.IGNORECASE)
    if match_cnae_p:
    ...
    match_bloco = re.search(padrao_bloco_sec, texto_completo, re.DOTALL | re.IGNORECASE)
    
    cnaes_found = []
    if match_bloco:
        bloco_texto = match_bloco.group(1)
        # Busca padrões CNAE com ou sem pontuação e também sequências de 7 dígitos
        lista_cnaes = re.findall(r'\d{2}\.?\d{2}-?\d-?\d{2}|\d{7}', bloco_texto)
        # normaliza (remove vazios e duplicatas mantendo ordem)
        seen = set()
        for c in lista_cnaes:
            if c and c not in seen:
                seen.add(c)
                cnaes_found.append(c)
    dados['cnaes_secundarios'] = cnaes_found

    # --- LIMPEZA: garantir que retornamos os originais (para exibição) e deixamos a limpeza
    # a cargo do fluxo principal usando apenas_numeros() — mas podemos já garantir que, se não houver,
    # tentamos encontrar padrões "apenas números" soltos no texto para não perder o dado.
    # Exemplos rápidos de fallback:
    if not dados['cnpj']:
        fallback = re.search(r'(\d{14})', texto_completo)  # CNPJ sem formatação: 14 dígitos
        if fallback:
            dados['cnpj'] = fallback.group(1)
    if not dados['cnae_principal']:
        fallback = re.search(r'(\d{7})', texto_completo)
        if fallback:
            dados['cnae_principal'] = fallback.group(1)
...
if not (df_nj is not None and df_cnae is not None and df_cnpj_list is not None):
    ...
    if arquivo_pdf:
        with st.spinner("Lendo e higienizando dados..."):
            ...
            cnaes_sec_map = {}
            for orig in dados['cnaes_secundarios']:
                if orig and str(orig).strip():
                    key = apenas_numeros(orig)
                    if key:  # evita chaves vazias
                        cnaes_sec_map[key] = orig
            ...
            with col_det:
                ...
                st.write(f"**Nat. Jurídica (raw):** {dados['nat_jur']} ➡️ `{nj_limpo}`")
                st.write(f"**CNAE Principal (raw):** {dados['cnae_principal']} ➡️ `{cnae_p_limpo}`")
                st.write(f"**CNPJ (raw):** {dados['cnpj']} ➡️ `{cnpj_limpo}`")
                st.write(f"**Secundários ({len(dados['cnaes_secundarios'])}):**")
                for orig in dados['cnaes_secundarios']:
                    st.text(f"{orig} -> {apenas_numeros(orig)}")
